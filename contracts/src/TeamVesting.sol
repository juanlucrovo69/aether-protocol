...
/**
 * @title TeamVesting
 * @notice Vesting contract for Aether Protocol team allocation.
 * @dev 12-month cliff + 36-month linear vesting. Non-revocable after schedule creation.
 */
contract TeamVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public constant DURATION = 1095 days;   // 36 months
     ...
     * @dev Can only be called once per beneficiary. startTime usually = TGE.
     */
    function createSchedule(address beneficiary, uint256 amount, uint256 startTime) external onlyOwner {
        ...
        require(startTime >= block.timestamp - 1 days, "Start time too old"); // small tolerance
